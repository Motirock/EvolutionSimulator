...
public class Game {
    ...
    public Game(GamePanel gp) {
        ...
        for (int i = 0; i < maxAgents; i++) {
            agents.add(new Agent(Math.random()*1600*scale, Math.random()*900*scale, new NeuralNetwork(inputSize, outputSize, layerSizes), new Color((int) (Math.random()*255), (int) (Math.random()*255), (int) (Math.random()*255))));
        }
            ...
            for (int i = 0; i < agents.size(); i++) {
                ...
                if (a.fitness > 1) {
                    ...
                    if (b < 0)
                        ...
                        b = 255;
                    agents.add(new Agent(a.position.x, a.position.y, a.nn.getMutated(), new Color(r, g, b)));
                    a.fitness--;
                }
            }
        }
    ...
    public void draw(Graphics2D g2, double GS) {
        ...
        if (agents.size() > 0) {
            ...
            for (Agent a : agents) {
                if (a.timeAlive > bestAgent.timeAlive) {
                    bestAgent = a;
                }
            }
            if (!bestAgent.alive)
